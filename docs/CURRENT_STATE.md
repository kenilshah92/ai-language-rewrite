...
The model picker is shared by both modes and preserves its current value across mode switches. GPT-5.6 Terra is the configured default and is listed first, followed by Luna for routine cost-sensitive work, Sol for complex professional work, and Astra for exceptional cases.
