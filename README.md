...
Open http://127.0.0.1:8000. Select one or more files and a configured model. The app processes jobs in the background and offers a rewritten document plus a JSON report. The default model is `gpt-5.6-luna`; account API access is required. The shared selector keeps its choice while switching between file and text rewrites. It offers GPT-5.6 Luna, GPT-5.6 Terra, and GPT-5.6 Sol in that order. Astra and other model IDs are intentionally unavailable for this app.
